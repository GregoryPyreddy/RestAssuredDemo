...
public class Builder extends RequestBuilder {
	...
	public void user_should_get_Expected_Status_code_and_Response_should_match_with(int ResponseCode,
			String ResponseFile) throws Throwable {
		
		if (response.getStatusCode() == ResponseCode) {
			
			if (Body.equalsIgnoreCase(ResponseFile)) {
				GenerateExtentReport.TestAssert(true, "Then user is getting Expected Result Status Code: "
						+ response.getStatusCode() + " and response body");
			} else {
				GenerateExtentReport.TestAssert(false, "Then user is getting Expected Result Status Code: "
						+ response.getStatusCode() + " but invalid response body");
